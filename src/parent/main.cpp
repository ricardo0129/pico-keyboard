...
void process_key_press(bool is_pressed, uint64_t now, uint8_t keycode,
  ...
  if (is_pressed !=
      current.is_pressed) { // keystate has changed, add to processing queue
    KeyEvent event = {.is_pressed = is_pressed, .keycode = keycode};
...
static void send_hid_report(uint8_t report_id) {
  ...
  /*
  if(state != key_states[i][j].is_pressed && now - key_states[i][j].last_changed
  > DEBOUNCE_US) { key_states[i][j].is_pressed = state;
  ...
  else if(state && now - key_states[i][j].last_changed > KEY_HOLD_US &&
  key_count < MAX_KEY_COUNT) { uint8_t chr = left_layout[i][j]; if(
  conv_table[chr][0] ) modifier = KEYBOARD_MODIFIER_LEFTSHIFT;
      keycode[key_count++] = conv_table[chr][1];
      changed = true;
  }
  */
...
int main() {
  ...
  received_event.is_pressed, received_event.keycode); j += 1;
      }
  }
  */
  int row_to_pin[4] = {27, 28, 26, 22};
  int col_to_pin[5] = {21, 4, 5, 6, 7};
  KeyBoard kb_left(row_to_pin, col_to_pin, left_layout_vec,
                   4, // rows
                   5  // cols
  );
